...
public class AStar
{
    ...
    public List<Node> FindPath(Node startNode, Node endNode, GridManager gridManager)
    {
        ...
        var openList = new PriorityQueue(); // 방문 안한거
        var closedList = new PriorityQueue(); // 방문 한거
        ...
        while (openList.Length != 0)
        {
            ...
            for (int i = 0; i < neighbors.Count; i++)
            {
                ...
                float costToMove = (node.pos - neighborNode.pos).magnitude; // 목적지까지 최단 거리
                float tentativeG = node.nodeTotalCost + costToMove; // 아직 결정하지 않은 추정거리
